...
public class DynamixelControlLoop {
	private static final Logger theLogger = LoggerFactory.getLogger(DynamixelControlLoop.class);
	...
	private void commandLoop() {
		while (myRunFlag) {
			try {
				...
				theLogger.warn("Recovering from error in Dynamixel control loop: ", t);
				TimeUtils.sleep(5);
			}
		}
	}
	...
	private synchronized void move() {
		...
		try {
			...
			if (!DynamixelMover.moveServos(myController, params, mySettings)) {
				theLogger.warn("There was an error moving the dynamixels.");
