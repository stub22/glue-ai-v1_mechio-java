...
public class RemoteGpioServiceClient<T extends SensorEventHeader>
	...
	private static final Logger theLogger = LoggerFactory.getLogger(RemoteGpioServiceClient.class);
	...
	public void setPinValue(int channel, boolean val) {
		...
		if (pin.direction == IN) {
			theLogger.warn("Unable to set pin value for channel {}, "
	...
	public void setReadPeriod(DeviceReadPeriodEvent<T> readPeriod) {
		if (readPeriod == null) {
			theLogger.error("Null read period.");
	...
	private void handPinEvent(ChannelBoolEvent<T> t) {
		...
		if (myInputValueSequenceId != seqId) {
			theLogger.warn("Unexpected sequenceId for Gpio value: {}", seqId);
		}
		GpioPin pin = myPins.get(chan);
		if (pin == null) {
			theLogger.warn("Received message for unknown gpio pin {}", chan);
			return;
		}
