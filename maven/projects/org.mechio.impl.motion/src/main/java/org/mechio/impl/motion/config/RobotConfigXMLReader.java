...
public class RobotConfigXMLReader
	...
	private static final Logger theLogger = LoggerFactory.getLogger(RobotConfigXMLReader.class);
	/**
	...
	public static ServoRobotConfig readConfig(BundleContext context, HierarchicalConfiguration xml) {
		...
		if (servos == null || servos.isEmpty()) {
			theLogger.error("Unable to find '{}' root element.", XML_JOINT_CONFIG);
			return null;
		}
		...
		for (HierarchicalConfiguration cc : (List<HierarchicalConfiguration>) controllers.configurationsAt(XML_CONTROLLER)) {
			...
			if (controllerContext == null) {
				theLogger.warn("Unable to read ServoControllerContext.");
				continue;
			}
	...
	private static void addServo(BundleContext context, HierarchicalConfiguration xml,
		...
		if (jointIdInt == null || controllerIdStr == null || servoIdStr == null) {
			if (jointIdInt == null) {
				theLogger.warn("Found {} element with no {} attribute.", XML_JOINT, XML_JOINT_JOINT_ID);
			}
			if (controllerIdStr == null) {
				theLogger.warn("Found {} element with no {} attribute.", XML_JOINT, XML_JOINT_SERVO_CONTROLLER_ID);
			}
			if (servoIdStr == null) {
				theLogger.warn("Found {} element with no {} attribute.", XML_JOINT, XML_JOINT_SERVO_ID);
			}
			return;
		}
	...
	private static ServiceReference getServoIdReader(BundleContext context,
		...
		try {
			...
			theLogger.warn("Invalid filter syntax: {}", filter, ex);
			return null;
		}
		if (refs == null || refs.length == 0) {
			theLogger.warn("Could not find ServoIdReader for {}", scId);
			return null;
		}
		return refs[0];
	}
	...
	private static ServoJointAdapter getServoJointAdapter(BundleContext context,
		...
		if (refs == null || refs.length == 0) {
			theLogger.warn("Could not find ServoJointAdapter for {}", scId);
			return null;
		}
