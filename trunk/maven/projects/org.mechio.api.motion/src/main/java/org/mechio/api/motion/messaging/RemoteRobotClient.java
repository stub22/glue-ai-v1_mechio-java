...
public class RemoteRobotClient {
	private static final Logger theLogger = LoggerFactory.getLogger(RemoteRobotClient.class);
	/**
	...
	private Boolean makeStatusRequest(String requestType, long timeout) {
		...
		if (resp == null) {
			theLogger.warn("Received null status for: {}, from: {}",
					requestType, myRobotId.getRobtIdString());
			return null;
		}
	...
	private Boolean makeStatusRequestForJoint(String requestType, JointId jointId, long timeout) {
		...
		if (resp == null) {
			theLogger.warn("Received null positions for: {}, from: {}",
					requestType, myRobotId.getRobtIdString());
			return null;
		}
		return resp.getStatusResponse();
	...
	private RobotDefinitionResponse makeDefinitionRequest(
			String requestType, long timeout) {
		theLogger.info("Making Robot Definition Request.  "
						+ "Request Type: {}, Timeout: {}, Robot: {}",
				requestType, timeout, myRobotId.getRobtIdString());
		RobotDefinitionResponse resp = makeBlockingRequest(
				RobotDefinitionResponse.class, requestType, timeout);
		if (resp == null) {
			theLogger.warn("Received null definition for: {}, from: {}",
					requestType, myRobotId.getRobtIdString());
			return null;
		}
		return resp;
	...
	private synchronized <T extends RobotResponse> T makeBlockingRequest(
		...
		if (clearedCount > 0) {
			theLogger.info("Cleared {} messages before making blocking request.  "
							+ "Request Type: {}, Timeout: {}, Robot: {}, Response Class: {}",
					clearedCount, requestType, timeout, myRobotId.getRobtIdString(),
					responseClass);
		}
		...
		theLogger.info("Robot Request Sent.  Fetching Response.  "
						+ "Request Type: {}, Timeout: {}, Robot: {}, Response Class: {}",
				requestType, timeout, myRobotId.getRobtIdString(),
				responseClass);
	...
	private <T extends RobotResponse> T fetchTypedResponse(
		...
		do {
			...
			if (resp == null) {
				theLogger.warn("Received null Message from Receiver");
			} else if (!isMatch(request, resp.getResponseHeader())) {
				theLogger.warn("Response does not match Request.  Ignoring response.");
			} else if (clazz.isAssignableFrom(resp.getClass())) {
				return (T) resp;
			} else {
				theLogger.info("Requested class ({}) does not Response class ({}).",
						clazz, resp.getClass());
			}
	...
	private boolean isMatch(RobotRequest req, RobotResponseHeader resp) {
		...
		if (!req.getRobotId().equals(resp.getRobotId())) {
			theLogger.info("Requested Robot Id ({}) does not match Response Robot Id ({}).",
					req.getRobotId(), resp.getRobotId());
			return false;
		} else if (!req.getRequestType().equals(resp.getRequestType())) {
			theLogger.info("Request Type ({}) does not match Response Type ({}).",
					req.getRequestType(), resp.getRequestType());
			return false;
        ...
        */
			...
			theLogger.info("Request Timestamp ({}) does not match Response Request Timestamp ({}).",
					req.getTimestampMillisecUTC(),
