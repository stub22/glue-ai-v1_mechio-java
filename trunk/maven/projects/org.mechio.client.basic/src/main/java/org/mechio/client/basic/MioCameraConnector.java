...
final class MioCameraConnector extends MioServiceConnector {
	...
	static Map<String, MioCameraConnector> theMioCameraConnectorMap = new TreeMap<>();

	static synchronized MioCameraConnector getConnector() {
		return getConnector(MechIO.getCameraContextId());
	}

	static synchronized MioCameraConnector getConnector(final String context) {
		if (!theMioCameraConnectorMap.containsKey(context)) {
			final MioCameraConnector mioCameraConnector = new MioCameraConnector();
			theMioCameraConnectorMap.put(context, mioCameraConnector);
		}

		return theMioCameraConnectorMap.get(context);
	}
