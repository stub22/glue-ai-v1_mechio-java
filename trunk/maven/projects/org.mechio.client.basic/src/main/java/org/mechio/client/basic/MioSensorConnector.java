...
final class MioSensorConnector extends MioServiceConnector {
	...
	static Map<String, MioSensorConnector> theMioSensorConnectorMap = new TreeMap<>();

	static synchronized MioSensorConnector getConnector() {
		return getConnector(MechIO.getSensorContextId());
	}

	static synchronized MioSensorConnector getConnector(final String context) {
		if (!theMioSensorConnectorMap.containsKey(context)) {
			final MioSensorConnector mioSensorConnector = new MioSensorConnector();
			theMioSensorConnectorMap.put(context, mioSensorConnector);
		}

		return theMioSensorConnectorMap.get(context);
	}
	...
	synchronized RemoteGpioServiceClient buildRemoteClient() {
		...
		RemoteGpioServiceClient<HeaderRecord> client = new RemoteGpioServiceClient<>(
