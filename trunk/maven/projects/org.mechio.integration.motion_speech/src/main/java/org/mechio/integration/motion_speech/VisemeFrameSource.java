...
public class VisemeFrameSource implements FrameSource<RobotPositionMap> {
	private static final Logger theLogger = LoggerFactory.getLogger(VisemeFrameSource.class);
	...
	private void cacheAllVisemePositions() {
		myVisemePositions = new EnumMap<>(Viseme.class);
	...
	private RobotPositionMap cacheVisemePositions(Viseme viseme) {
		...
		if (myVisemeManager == null) {
			theLogger.warn("Missing Viseme Manager for: {}", myRobotId);
			return null;
		}
		Map<Integer, NormalizedDouble> vals =
				myVisemeManager.getBindingValues(viseme);
		if (vals == null) {
			theLogger.warn("Missing Viseme Binding for: {}", viseme.name());
			return null;
		}
